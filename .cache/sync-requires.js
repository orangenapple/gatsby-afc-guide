// prefer default export if available
const preferDefault = m => m && m.default || m


exports.layouts = {
  "layout---index": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/.cache/layouts/index.js"))
}

exports.components = {
  "component---node-modules-gatsby-plugin-offline-app-shell-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/node_modules/gatsby-plugin-offline/app-shell.js")),
  "component---src-templates-post-template-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/src/templates/PostTemplate.js")),
  "component---src-templates-page-template-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/src/templates/PageTemplate.js")),
  "component---src-pages-contact-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/src/pages/contact.js")),
  "component---src-pages-index-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/src/pages/index.js")),
  "component---src-pages-search-js": preferDefault(require("/Users/georgiabarrie/gatsby-afc-guide/src/pages/search.js"))
}

exports.json = {
  "layout-index.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/layout-index.json"),
  "offline-plugin-app-shell-fallback.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/offline-plugin-app-shell-fallback.json"),
  "keeping-this-guide-offline.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/keeping-this-guide-offline.json"),
  "testing-milk-quality.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/testing-milk-quality.json"),
  "milk-transportation.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/milk-transportation.json"),
  "keeping-records.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/keeping-records.json"),
  "choosing-forages-to-cultivate.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/choosing-forages-to-cultivate.json"),
  "prevention-of-ketosis.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/prevention-of-ketosis.json"),
  "dairy-cattle-sheds-introduction.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/dairy-cattle-sheds-introduction.json"),
  "clean-milk-production.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/clean-milk-production.json"),
  "tail-rot.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/tail-rot.json"),
  "calf-rearing.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/calf-rearing.json"),
  "sleeping-sickness.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/sleeping-sickness.json"),
  "foot-and-mouth-disease.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/foot-and-mouth-disease.json"),
  "sore-mouth.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/sore-mouth.json"),
  "malignant-catarrhal-fever.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/malignant-catarrhal-fever.json"),
  "deworming.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/deworming.json"),
  "heifer-rearing.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/heifer-rearing.json"),
  "feeding-dry-cows.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/feeding-dry-cows.json"),
  "getting-better-prices.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/getting-better-prices.json"),
  "understanding-breeds.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/understanding-breeds.json"),
  "feeding-your-cow-basics.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/feeding-your-cow-basics.json"),
  "preventing-and-managing-bloat.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/preventing-and-managing-bloat.json"),
  "feeding-milking-cows.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/feeding-milking-cows.json"),
  "preventing-and-treating-milk-fever.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/preventing-and-treating-milk-fever.json"),
  "success.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/success.json"),
  "about.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/about.json"),
  "starters.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/starters.json"),
  "feeding-calf.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/feeding-calf.json"),
  "preventing-mastitus.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/preventing-mastitus.json"),
  "east-coast-fever-disease.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/east-coast-fever-disease.json"),
  "increasing-milk-production-part-1.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/increasing-milk-production-part-1.json"),
  "increasing-milk-production-part-2.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/increasing-milk-production-part-2.json"),
  "signs-for-a-cow-on-heat.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/signs-for-a-cow-on-heat.json"),
  "when-to-serve-and-ai.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/when-to-serve-and-ai.json"),
  "contact.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/contact.json"),
  "index.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/index.json"),
  "search.json": require("/Users/georgiabarrie/gatsby-afc-guide/.cache/json/search.json")
}